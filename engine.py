...
def run_full_analysis(product_name, inci_list_str, known_percentages_str):
    try:
        ...
        if prohibited_found:
            ...
            # BUG FIX: Ensure 4 values are returned on error to prevent crashing app.py
...
# --- STAGE 1 FUNCTIONS ---
# KEPT FROM YOUR FILE: Your improved product profile identification logic.
...
# KEPT FROM YOUR FILE: Your requested "Backward Fill & Normalize" percentage estimation logic.
def estimate_percentages(inci_list, profile, markers, known_percentages):
    percentages = {name: 0.0 for name in inci_list} # Initialize with 0.0
...
# RESTORED: Improved function to prevent misidentification of common ingredients like "Water".
...
# KEPT FROM YOUR FILE: Your requested "Stage 4" role identification logic.
...
# RESTORED: The complete, "smart" scoring and narrative generation logic to fix low scores.
...
# RESTORED: Improved and robust routine matching logic.
